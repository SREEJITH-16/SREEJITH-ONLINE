...
export function useInView<T extends HTMLElement>(threshold = 0.12) {
  ...
  useEffect(() => {
    ...
    // Safety net: never let content stay permanently hidden if the observer is
    // slow, misses its window, or JS hydration is delayed.
    const fallback = window.setTimeout(() => {
      const rect = el.getBoundingClientRect();
      const nearViewport = rect.top < window.innerHeight + 400 && rect.bottom > -400;
      if (nearViewport) setVisible(true);
    }, 1200);
